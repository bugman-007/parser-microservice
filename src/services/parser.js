...
export class AIParserV3 {
  constructor(options = {}) {
    ...
    this.uploadDir =
      options.uploadDir || process.env.UPLOAD_DIR || "/opt/parser/uploads";
  ...
  async parseFile(jobId, filePath, options = {}) {
    ...
    try {
      ...
      console.log(
        `📐 Card dimensions: ${dimensions.width}×${dimensions.height}mm`
      );

      // 4. Extract OCG layers (main parsing logic)
      const layers = await this.extractOCGLayers(
        pdfDoc,
        filePath,
        assetsDir,
        jobId
      ...
      const albedoMaps = await this.generateAlbedoMaps(
        filePath,
        assetsDir,
        dimensions
      );
      ...
      // 9. Save results
      await fs.writeJson(path.join(jobDir, "result.json"), manifest, {
        spaces: 2,
      });

      console.log(
        `✅ Parsing completed successfully in ${manifest.parsing.parseTime}ms`
      );
      console.log(
        `🎯 Confidence: ${(manifest.parsing.confidence * 100).toFixed(1)}%`
      );
  ...
  async extractOCGLayers(pdfDoc, filePath, assetsDir, jobId) {
    ...
    try {
      ...
      const ocPropsRef = catalog.lookup(
        catalog.context.obj({
      ...
      if (!ocPropsRef || !this.enableOCG) {
        console.log(
          "📄 No OCG layers found or OCG disabled, using fallback method"
        );
      ...
      console.log(
        `📋 Found ${layerNames.length} potential layers:`,
        layerNames
      );
      ...
      for (let i = 0; i < layerNames.length; i++) {
        ...
        if (effectInfo) {
          console.log(
            `🎨 Processing effect layer: "${layerName}" (${effectInfo.type})`
          );

          try {
            // Generate mask for this specific layer
            const maskResult = await this.renderLayerMask(
              filePath,
              layerName,
              i,
              assetsDir
            ...
            if (maskResult) {
              const bounds = await this.extractBoundsFromMask(
                maskResult.maskPath
              );
            ...
            console.warn(
              `⚠️ Error processing layer "${layerName}":`,
              error.message
            );
          }
        } else {
  ...
  async renderLayerMask(filePath, layerName, layerIndex, assetsDir) {
    ...
    const outputPath = path.join(
      assetsDir,
      `${sanitizedName}_${layerIndex}.png`
    );

    try {
      // Method 1: Try pdftocairo with OCG isolation (requires advanced tools)
      const method1Success = await this.tryAdvancedOCGRendering(
        filePath,
        layerName,
        outputPath
      );
      ...
      console.error(
        `❌ Mask rendering failed for ${layerName}:`,
        error.message
      );
      return null;
    }
  ...
  async postProcessMask(maskPath, layerName) {
    try {
      ...
      if (effectInfo?.type === "foil") {
        ...
        processedImage = processedImage.threshold(128).blur(0.5);
      } else if (effectInfo?.type === "emboss") {
        // Create height map for embossing
        processedImage = processedImage.greyscale().normalise();
      }
  ...
  async generateFallbackMask(assetsDir, effect, index) {
    try {
      ...
      const maskPath = path.join(
        assetsDir,
        `fallback_${effect.type}_${index}.png`
      );
  ...
  detectEffectLayer(layerName) {
    ...
    for (const [effectType, config] of Object.entries(patterns)) {
      if (config.keywords.some((keyword) => name.includes(keyword))) {
        // Determine subtype
        let subtype = "default";
        for (const [sub, subKeywords] of Object.entries(
          config.subtypes || {}
        )) {
        ...
        // Determine side
        const side =
          name.includes("back") || name.includes("rear") ? "back" : "front";
